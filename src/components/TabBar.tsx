...
const TabBar = (props:BottomTabBarProps): React.JSX.Element => {
  ...
  const [selectedTabDimensions, setSelectedTabDimensions] = useState({width: 0, height: 0});
          ...
          >
            {
              ...
              index === 1 ? <CoursesIcon/> :
              index === 2 ? <QuizIcon/> :
              index === 3 ? <UserIcon/> : null
            }
            <Text style={[styles.tabBarText, {display: isFocused ?  'flex':'none', color: colors.primary000}]}>{label}</Text>
          </Pressable>
        );
      })}
    </Animated.View>
